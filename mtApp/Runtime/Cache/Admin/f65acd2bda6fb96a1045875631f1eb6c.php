...
<body>
<?php
 $msg = isset($_GET['msg']) ? $_GET['msg'] : ''; if(!empty($msg)){ echo '<input type="hidden" name="ref" value="ref">'; } ?>
<div class="pd-20">
	<form action="<?php echo U('Picture/addPic');?>" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
		<div class="row cl">
			<label class="form-label col-2"><span class="c-red">*</span>图片标题：</label>
			<div class="formControls col-10">
				<input type="text" class="input-text" value="" placeholder="" id="" name="picname" required>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-2"> <span class="select-box">
                ...
                <?php if(is_array($typenameList)): foreach($typenameList as $key=>$val): ?><option value="<?php echo ($val["id"]); ?>"><?php echo ($val["typename"]); ?></option><?php endforeach; endif; ?>
				</select>
				</span>
            </div><span style="color:red;">更换前请先下架所对应的图片！！</span>
		</div>

		<div class="row cl">
			<label class="form-label col-2">图片摘要：</label>
			<div class="formControls col-10">
				<textarea name="des" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="textarealength(this,200)" required></textarea>
				...
				<div class="uploader-list-container"> 
					<div class="queueList">
						<div id="dndArea" class="placeholder">
							...
							<p>
                            <?php
 $str = isset($_GET['str'])?$_GET['str']:''; if(!empty($str)){ echo '<span style="color:red">请上传图片！</span>'; }else{ echo '或将照片拖到这里'; } ?>
                            </p>
						</div>
					</div>
        ...
        <div class="row cl">
            <label class="form-label col-offset-1 col-2" style="color:red"><?php echo ($map["msg"]); ?></label>
        </div>
		<div class="row cl">
...
$(function(){
	...
	var uploader = WebUploader.create({
		auto: true,
		swf: '/mtadmin/Public/lib/webuploader/0.1.5/Uploader.swf',
	
		// 文件接收服务端。
		server: "<?php echo U('Picture/upPic');?>",
    ...
    $(function() {
        ...
        function addFile( file ) {
            ...
            uploader.on('uploadSuccess', function( file,json ){
                //console.log(json);
                if(json){
                    $wrap.append('<input type="hidden" name="picpath" value="'+json+'">');
                }
            });
