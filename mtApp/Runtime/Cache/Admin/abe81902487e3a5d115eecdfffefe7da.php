...
<div class="pd-20" style="padding-top:20px;">
  <p class="f-20 text-success">欢迎登录 <span class="f-20">萌田网络</span>后台管理系统</p>
  <p id=''>您的登录次数为：<span id="loginnum"></span> 次 </p>
  <p>上次登录IP：<span id="ip"></span>   上次登录时间：<span id="time"></span></p>
...
<footer class="footer">
  <p>感谢jQuery、layer、laypage、Validform、UEditor、My97DatePicker、iconfont、Datatables、WebUploaded、icheck、highcharts、bootstrap-Switch<br>Copyright &copy;2015 H-ui.admin v2.3 All Rights Reserved.<br>
    本后台系统由<a href="http://www.yotoai.com/" target="_blank" title="萌田网络">萌田网络</a>提供技术支持</p>
</footer>
...
$(function(){
  $.ajax({
    type:'post',
    url:"<?php echo U('Index/getBaseInfo');?>",
    success:function(data){
      if(data){
        $('#loginnum').text(data.loginnum);
        $('#ip').text(data.loginip);
        $('#time').text(data.logintime);
      }
    }
  })
});
