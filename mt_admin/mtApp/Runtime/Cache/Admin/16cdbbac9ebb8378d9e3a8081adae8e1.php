...
<div class="pd-20">
	<form action="" method="post" class="form form-horizontal" id="form-admin-add">
		...
		<div class="row cl">
			...
			<div class="formControls col-5">
				<input type="password" placeholder="密码长度应为 6-18 位由数字字母组成..." autocomplete="off" value="" class="input-text" name="newpassword">
			</div>
			<div class="col-4"> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-5">
				<input type="password" placeholder="确认密码" autocomplete="off" class="input-text" name="newpassword2">
			</div>
			<div class="col-4"> </div>
		</div>
...
$(function(){
	...
	$("#user-name").blur(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Admin/checkName');?>",
...
function subform()
{
	...
	var pass = $.trim($("input[name='newpassword']").val());
	if(pass == ''){
	...
	var pass1 = $.trim($("input[name='newpassword2']").val());
	if(pass1 == ''){
	...
	$.ajax({
		type:"post",
		url:"<?php echo U('Admin/addAdmin');?>",
		data:"username="+username+"&password="+pass1+"&telephone="+telephone+"&email="+email+"&grade="+grade+"&des="+des,
		success:function(data){
			if(data.flag == true){
