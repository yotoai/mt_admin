	...
	<form action="" method="post" enctype="multipart/form-data" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
			...
			<div class="formControls col-7">
				<input type="text" class="input-text" value="" placeholder="" id="" name="comname" required>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-2">URL：</label>
			<div class="formControls col-7">
				<input type="text" class="input-text" value="" placeholder="" id="url" name="url" required>
			</div>
			<sapn id="msg">请输入正确的url地址,如：https://www.baidu.com</sapn>
		</div>
		
		<div class="row cl">
			<label class="form-label col-2">LOGO上传：</label>
...
$(function(){
	...
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server: "<?php echo U('Flinks/upLogo');?>",
    ...
    $(function() {
        ...
        function addFile( file ) {
            ...
            if ( file.getStatus() === 'invalid' ) {
                ...
                uploader.makeThumb( file, function( error, src ) {
                    ...
                    if( isSupportBase64 ) {
                        ...
                        $.ajax('/mtadmin/Public/lib/webuploader/0.1.5/server/preview.php', {
	                            method: 'POST',
	                            data: src,
	                            dataType:'json',
	                            success:function(data){
	                            	console.log(data);
	                            	if(data){
	                            		$wrap.append('<input type="hidden" name="image" value="'+data.fileName+'">');
	                				}
	                            }
    		...
    		uploader.on('uploadSuccess', function(file, json){
	                ...
	                console.log(json);
	                if(json){
	                     $wrap.append('<input type="hidden" name="image" value="'+json+'">');
	                }
    		});
...
// });
	$('#url').blur(function(){
	    ...
	    var strRegex = "^((https|http|ftp|rtsp|mms)?://)" + "?(([0-9a-z_!~*'().&=+$%-]+: )?[0-9a-z_!~*'().&=+$%-]+@)?" + "(([0-9]{1,3}\.){3}[0-9]{1,3}" + "|" + "([0-9a-z_!~*'()-]+\.)*" + "([0-9a-z][0-9a-z-]{0,61})?[0-9a-z]\." + "[a-z]{2,6})" + "(:[0-9]{1,4})?" + "((/?)|" + "(/[0-9a-z_!~*'().;?:@&=+$,%#-]+)+/?)$"; 
	     ...
	     //re.test()
	           if (re.test(str_url)){
	           		$("#msg").empty().append('<span class="Validform_checktip Validform_right"></span>');
	               	$("#sub").attr('onClick','');
	           }else{ 
	               	$("#msg").empty().append('<span style="color:red">格式有误！</span>'); 
